...
def value_iter(mdp, reward):
    ...
    """
    policyT = {
        s: None for s in mdp.states
    }  # initialize the policy to take None for every state
    ...
    while True:
        ...
        for s in set(mdp.states):
            ...
            for a in mdp.actions(s):
                Q[(s, a)] = reward[(s, a)] + sum(
                    [mdp.P(s, a, next_s) * Vstate[next_s] for next_s in mdp.states]
                )
                if Q[(s, a)] >= optimal:
                    optimal = Q[(s, a)]
                    act = a
...
def best_policy(mdp, AEC=None):
    ...
    for state in mdp.states:
        U[state] = LpVariable(
            "U" + str(state), 0
        )  # for each state, assign a state value variable.
    ...
    for state in mdp.states:
        for act in mdp.actions(state):
            C = LpAffineExpression(
                [
                    (U[next_state], mdp.P(state, act, next_state))
    ...
    for state in set(mdp.states) - Win:
        ...
        for act in mdp.actions(state):
            Q[(state, act)] = np.inner(
                mdp.T(state, act),
                np.array([U[next_state].varValue for next_state in mdp.states]),
            )
    ...
    return (
        ...
        Q,
        V,
    )  # the system will switch between policy1 and 2. When the state is within AEC, policy 1 will be taken, otherwise policy2 will be taken.
...
def E_state_value_iter(mdp, epsilon, AEC=None):
    ...
    while e > epsilon:
        ...
        for s in set(mdp.states) - Win:
            ...
            #        print "iteration: {} and the state value is {}".format(t, Vstate1)
            e = abs(
                max([Vstate1[s] - Vstate[s] for s in mdp.states])
            )  # the absolute error
...
def balanced_wandering(mdp, s):
    act = random.choice(
        list(mdp.actions(s))
    )  # randomly choose an action from the set of enabled actions.
