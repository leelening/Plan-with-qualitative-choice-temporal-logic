...
class MDP(object):
    ...
    def a_array(a: int) -> np.array:
        """
        Convert the int action to the 
        """
        if a == 0:
    ...
    def deterministic_transition(self, s: tuple, a: int):
        """
        ...
        :return: (tuple)
        """
        next_state = tuple(map(lambda x, y: x + y, s, self.a_array(a)))
        return next_state
