...
class MDP(object):
    ...
    def construct_states(self):
        self.states = np.asarray(product(range(self.grid_world_size[0]), range(self.grid_world_size[1])))
    ...
    def possible_neighbors(self, s, a):
        """
        This function returns the possible neighbors.
        ...
        """
        if a == 0:
            neighbors = [s + np.array([0, 1]), s + np.array([-1, 0]), s + np.array([1, 0])])
        elif a == 1:
            neighbors = [s + np.array([1, 0]), s + np.array([0, 1]), s + np.array([0, -1])])
        elif a == 2:
            neighbors = set([s + np.array([0, -1]), s + np.array([-1, 0]), s + np.array([1, 0])])
        else:
            neighbors = set([s + np.array([-1, 0]), s + np.array([0, 1]), s + np.array([0, -1])])
        neighbors = s
        return neighbors
    ...
    def __str__(self):
        ...
        :return: the information of the mdp
        :rtype: str
        """
        tmp = "-" * 10 + 'The transition matrix of the robot' + "-" * 10  + "\n"
